...
//        var regex4 = new Regex(@"RS\.\s([A-Za-z\s]+Rupees\sAnd\sZero\sPaisa\sOnly)", RegexOptions.IgnoreCase);
...
// BELOW CODE FOR FIND CONTAINER NUMBER
...
namespace InvoiceOCRApp2.Controllers
{
    public class SalesController : Controller
    {
        private readonly ApplicationDbContext _context;
        ...
        private async Task<(decimal, string)> GetAmountFromOCR(string filePath)
        {
            ...
            if (result == null || result.ParsedResults == null ||
                ...
                string.IsNullOrEmpty(result.ParsedResults[0].ParsedText))
            {
